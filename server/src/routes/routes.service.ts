...
export class RoutesService {
  ...
  async createRoute(route: Route): Promise<Route> {
    const g = await this.guideRepository.findOne({where: { name: route.guide.name }})
    const r = await this.regionRepository.findOne({where: { name: route.region.name }})
    const m = await this.mapRepository.findOne({where: { id: route.map.id }})
    
    return await this.routesRepository.save({...route, guide: g, region: r, map: m});
  }
}
